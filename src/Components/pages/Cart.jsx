...
import { emptyCart, newOrderedItem, removeItem } from '../../Slice';
...
const Cart = (props) => {
  ...
  const cartData = useSelector(state => state.Storedata.cart);
  const [cartItems, setCartItems] = useState(cartData);
  const [totalPrice, setTotalPrice] = useState(0);
  const add = useSelector(state => state.UserData.userAddress);
  ...
  function handleRemove(id) {
    ...
    setCartItems(ex => ex.filter((i) => i.id != id));
  }
  ...
  const handleOrder = () => {
    ...
    dispatch(newOrderedItem(cartItems));
    setCartItems([]);
    dispatch(emptyCart());
  };
