...
const Product = (props) => {
    ...
    const [inWishList, setInWishList] = useState(isPresent());
    ...
    const handleAddItemtoWishlist = ({ id, name, price, img, rating }) => {
      let flag = true;
    ...
    const addItem = () => {
      ...
      setItemInCart(true);
      const cartCount = Store.getState().Storedata.cart
      setCartItemsCount(cartCount.length);
    ...
    useEffect(() => {
      window.scrollTo(0, 0);
      const cartItems = Store.getState().Storedata.cart
